...
def countries_list(request):
    if request.method =='GET':
        ...
        countries_sereializer = CountriesSerializer(countries, many=True)
        return JsonResponse(countries_sereializer.data,safe=False)

    elif request.method == 'POST':
        countries_data = JSONParser().parse(request)
        countries_sereializer = CountriesSerializer(data=countries_data)
        if countries_sereializer.is_valid():
            countries_sereializer.save()
            return JsonResponse(countries_sereializer.data, status=status.HTTP_201_CREATED)
        return JsonResponse(countries_sereializer.errors, status = status.HTTP_400_BAD_REQUEST)
